...
public class Evaluation {
  ...
  public static boolean esitmate(Classifier classifier, String instance) throws Exception{
    config(classifier);
	String hypothesis = classifier.classifyInstance(instance);
	String label = (class_index == 1)?getClass(instance):getClassAtHead(instance);
	return hypothesis.equals(label);
  }

  public static boolean[] testClassifier(Classifier classifier, String testset) throws IOException{
	config(classifier);
  ...
  public static boolean[] check(String[] labels, String dataset) throws Exception{
  ...
  public static double jacobCoeffcient(boolean[] estimates1, boolean[] estimates2){
	double value = 0.0;
	...
	for(int i=0;i<estimates1.length;i++){
	  if(estimates1[i] == true){
		f1++;	
		if(estimates2[i] == true)  // Given both the estimates are equal to true, then f11 add 1
          f11++;			  
	  }
	}
	value = (double) f1/f11;
