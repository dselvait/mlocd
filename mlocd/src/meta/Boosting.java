...
public class Boosting extends Classifier {
  ...
  public Boosting(Classifier learner){this.learner=learner;}
...
//				updateDataWeights(weights, evaluations);
  ...
  public void build(String dataset){
	try{
	  ...
	  for(int i =1;i<times+1;i++){
		...
		boolean[] evaluations=utils.Evaluation.testClassifier(learner,dataset);
		Classifier trained = learner.copy();
		//rename the trained weak learner
		String newname = this.location+"/classifier"+Integer.toString(i);
		rename(trained,newname);
		updateDataWeights(weights,evaluations);
		//if(i==3)  // debug code
  ...
  private void initEnvironment(){
	if(this.location==null) 
	  this.location=System.getProperty("user.dir")+"/"+"BoostingClassifier";
	else this.location=this.location+"/"+"BoostingClassifier";
  ...
  private void updateDataWeights(double[] weights, boolean[] list){
	try{
	  ...
	  double sumError=0.0;
	  for(boolean b:list) {
	   if(!b) sumError += weights[i];
	   i++;
	  }
	  double m=sumError/(1-sumError);
	  i=0;
	  for(boolean b:list){
	   if(b) weights[i]=m*weights[i];
	   i++;
	  }
	}catch(IndexOutOfBoundsException e){
	   //throw new IllegalArgumentException("The size of weights vctor is incorrect");	
	   e.printStackTrace();
	} 
  }
  ...
  static private void rename(Classifier classifier, String newname)throws FileNotFoundException{	 
	int i=0;
	  ...
	  try{
	    ...
	    break; // If the FileNotFoundException is not thrown, then the skip the loop
         ...
         try{
		   ...
		   Thread.sleep(1000);   //sleep for 1 seconds to give time to gernerate the classifier
		   continue;
...
//	}
  ...
  private void cleanup(){
	...
	System.out.println("The Boosting environment has been cleaned up.");
	   ...
	   System.err.println("Fail to clean up the Boosting classifier environment.");
	   e.printStackTrace();
	   //System.exit(1);
   }    
  }
